...
import { View, Text, Image, FlatList, Pressable, SafeAreaView, ScrollView } from 'react-native';
import { useNavigation } from '@react-navigation/native';

import { getPopularMovies, getAllMovies } from '../../api/movies.api';
...
const Movies = () => {
  ...
  const MovieColumn = ({ id, posterImage, title }) => (
    <Pressable style={styles.movieColumnContainer} onPress={() => navigation.navigate("Movie", { id })}>
  ...
  return (
    ...
    <View style={styles.container}>
      <ScrollView>
        ...
        <FlatList
          data= {popularMovies}
          renderItem={value => <MovieColumn id={value.item.id} posterImage={value.item.posterImage} title={value.item.title}  />}
          horizontal
        />
        ...
        <FlatList 
          data={movies}
          renderItem={value => (<MovieRow image={value.item.image} title={value.item.title} characters={value.item.characters} genre={value.item.genre} release={value.item.release} />)}
        />
      </ScrollView>
      </View>
    </SafeAreaView>
